...
**强制规则**：
- 报告**只能有这四部分**，不允许新增"工程债检测""可执行建议""SOTA 对标深度解析"等独立模块——这些都已下沉到"三、七大模块深度评价"的子节
- "三、七大模块深度评价"每个里程碑必须截取真实代码（用 CODE REFERENCE 格式 ``startLine:endLine:filepath``）
- "四、整体评分"的评分维度必须严格对应"二、七大模块阶段评估"的七大里程碑，权重见第九节
...
**精简版强制规则**：
- 报告只能有 4 大块（与完整版相同）
- "三、七大模块深度评价"每个里程碑**不需要截取代码**，只需要用一句话描述"被评价 Agent 的设计思路"
- "三、七大模块深度评价"每个里程碑**不需要概念扩展卡片**
- "二、七大模块阶段评估"的"判定依据"只需要一句话说明，不需要代码证据
...
>
> **强制使用规则**："三、七大模块深度评价"每个里程碑的 3.x.1（类比解释）和 3.x.4（概念扩展卡片）**必须**从下表中引用至少一条对应类比；如本表没有合适条目，必须**新增一条**到本表，再在报告中引用——确保概念辅助学习自动进入报告。
...
> **使用规则**：本阶梯用于"二、七大模块阶段评估"中给被评价 Agent 横向定位。每次使用前**必须用 WebSearch 验证最顶端范式**是否仍是当前 SOTA。
...
| **代码截取强制** | "三、七大模块深度评价"每个里程碑的"被评价 Agent 怎么做的"**必须**用 CODE REFERENCE 引用真实代码（含起止行号） |
...
| **代码截取禁止** | "三、七大模块深度评价"每个里程碑的"被评价 Agent 的设计思路"**不需要**截取代码，只需要一句话描述 |
