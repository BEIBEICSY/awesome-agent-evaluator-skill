...
**强制规则**：
- 报告第三部分每个里程碑的 SOTA 对标，**必须**在当次会话中通过 WebSearch 验证至少一项最新事实（如版本号、API 变更、新发布的论文/产品）
...
**询问时必须包含的说明**：
- **完整版**：适合想要深入理解 Agent 架构设计的用户，报告较长，包含大量代码示例和类比解释。适合想要学习"为什么这样设计"的用户。
- **精简版**：适合只想要"结论+下一步"的用户，报告较短，不包含代码细节，但保留所有评估结论和改进建议。适合不愿意阅读代码的用户。

**用户选择后的处理**：
- 如果用户选择"完整版" → 按完整版流程执行（步骤 1-5）
- 如果用户选择"精简版" → 按精简版流程执行（见下文"精简版执行流程"）

---

### 步骤 1：通读项目所有 md / README / docs（强制！）
...
**强制规则**：
- 报告**只能有这四部分**，不允许新增"工程债检测""可执行建议""SOTA 对标深度解析"等独立模块——这些都已下沉到 第三部分的子节
- 第 三 部分 每个里程碑必须截取真实代码（用 CODE REFERENCE 格式 ``startLine:endLine:filepath``）
- 第 四 部分 的评分维度必须严格对应  二、的七大里程碑，权重见第九节

---

### 精简版执行流程（用户选择"精简版"时执行）

**核心差异**：精简版**不截取代码、不逐行解读**，只保留设计思路和改进建议。目标是让不愿意阅读代码的用户也能理解评估结论和下一步改进建议。

**执行步骤**：

#### 步骤 1-3：与完整版相同
- 步骤 1：通读项目所有 md 文档（强制）
- 步骤 2：识别项目类型
- 步骤 3：执行 SOTA 搜索（强制）

#### 步骤 4：读取项目代码（精简版调整）
- **只读取关键文件**，不需要截取代码片段
- 重点提取：**设计思路、架构模式、关键决策**，而非具体实现细节
- 不需要记录代码行号，只需要理解"被评价 Agent 是怎么设计的"

#### 步骤 5：生成精简版报告（严格 4 大块结构）
...
**精简版强制规则**：
- 报告只能有 4 大块（与完整版相同）
- 第 3 块每个里程碑**不需要截取代码**，只需要用一句话描述"被评价 Agent 的设计思路"
- 第 3 块每个里程碑**不需要概念扩展卡片**
- 第 2 块的"判定依据"只需要一句话说明，不需要代码证据
- **必须保留**：差距等级（红/黄/绿）、类比解释、建议的时间划分（立即/短期/中期）
...
>
> **强制使用规则**：第 三 部分每个里程碑的 3.x.1（类比解释）和 3.x.4（概念扩展卡片）**必须**从下表中引用至少一条对应类比；如本表没有合适条目，必须**新增一条**到本表，再在报告中引用——确保概念辅助学习自动进入报告。
...
> **重要说明**：本 Skill 支持两种报告版本——**完整版**（含代码细节）和**精简版**（仅结论和建议）。在开始评估前，必须先询问用户需要的版本（见第四节步骤 0.5）。
...
| **代码截取禁止** | 第 3 块每个里程碑的"被评价 Agent 的设计思路"**不需要**截取代码，只需要一句话描述 |
